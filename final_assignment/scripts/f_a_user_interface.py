...
target_pos_list_ = [[-4,-3],[-4,2],[-4,7],[5,-7],[5,-3],[5,1]]   ### Initializing the list containing all the possible robot's target positions
...
def main():
    ...
    global  srv_client_user_interface_, target_reached_status, srv_client_wall_follower_
	...
	if (choosen_State == 1):
		...
		resp = srv_client_wall_follower_(False)         ### Closing the 'following_wall' script for cases in which that running prior to this state. 
                ...
                print("The new target position is (" + str(target_pos_list_[random_index_-1][0]) + ", " + str(target_pos_list_[random_index_-1][1]) + ")")
		...
		move_base.goal.target_pose.pose.position.x = target_pos_list_[random_index_-1][0]
		move_base.goal.target_pose.pose.position.y = target_pos_list_[random_index_-1][1]
		...
		print("1.(-4,-3) 2.(-4,2) 3.(-4,7) 4.(5,-7) 5.(5,-3) 6.(5,1)")
                ...
                print("The new target position is ("+ str(target_pos_list_[user_input-1][0]) + ", " + str(target_pos_list_[user_input-1][1]) + ")")
		...
		move_base.goal.target_pose.header.frame_id = "map"
		move_base.goal.target_pose.pose.orientation.w = 1
		move_base.goal.target_pose.pose.position.x = target_pos_list_[user_input-1][0]
		move_base.goal.target_pose.pose.position.y = target_pos_list_[user_input-1][1]
		publish_new_target_.publish(move_base)
